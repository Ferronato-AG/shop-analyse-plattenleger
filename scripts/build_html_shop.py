...
<div class="tools"><input id="q" type="search" placeholder="Suchen (Name, Text, Art.-Nr.)" size="30"><select id="fs"><option value="">Alle Sparten</option>{"".join(f'<option>{e(s)}</option>' for s in SPORDER)}</select><select id="fk"><option value="">Alle Hauptkategorien</option>{"".join(f'<option>{e(k)}</option>' for k in sorted(set(o["hauptkategorie"] for o in F)))}</select><label><input type="checkbox" id="ft"> nur Top</label><span id="cnt" class="count"></span></div>
<div class="wrap"><table><thead><tr><th>Sparte</th><th>Hauptkategorie</th><th>Unterkategorie</th><th>Top</th><th>Rel.</th><th>Produktname</th><th>Bild</th><th>Beschreibung</th><th>Feature / USP</th><th>Sekundäre Info</th><th>Link</th><th>CHF</th><th>Art.-Nr.</th><th>Alter Shop-Pfad</th></tr></thead><tbody id="tb"></tbody></table></div>
...
const tb=document.getElementById('tb'),q=document.getElementById('q'),fs=document.getElementById('fs'),fk=document.getElementById('fk'),ft=document.getElementById('ft'),cnt=document.getElementById('cnt');
